...
class PlotsPublisherNode{
 ...
 class TrajTrail{
   ...
   void update(){
	...
	listener.waitForTransform("/world", "/av1", now, ros::Duration(5.0));
	listener.lookupTransform("/world", "/av1", ros::Time(0), transform);
	listener.waitForTransform("/world", "/av2", now, ros::Duration(5.0));
	listener.lookupTransform("/world", "/av2", ros::Time(0), transform);
